...
addon.get("/:language?/catalog/:type/:id/:extra?.json", async function (req, res) {
  ...
  const metas = search ? await getSearch(type, language, search) : id === "tmdb.trending" ? await getTrending(type, id, language, genre, page) : await getCatalog(type, id, language, genre, page) 
  const cacheOpts = {
