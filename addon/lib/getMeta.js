...
const buildMovieResponse = async (res, type, language, tmdbId, config = {}) => {
  ...
  const logo = rpdbMediaTypes?.logo ? await Utils.parseMediaImage(type, tmdbId, null, language, rpdbkey, "logo", rpdbMediaTypes) : await getLogo(tmdbId, language, res.original_language).catch(e => {
...
const buildTvResponse = async (res, type, language, tmdbId, config = {}) => {
  ...
  const rpdbMediaTypes = config.rpdbMediaTypes || null;
  const logo = rpdbMediaTypes?.logo ? await Utils.parseMediaImage(type, tmdbId, null, language, rpdbkey, "logo", rpdbMediaTypes) : await getTvLogo(res.external_ids?.tvdb_id, res.id, language, res.original_language).catch(e => {
