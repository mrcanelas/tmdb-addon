...
export const identityPreferencesRoutes: FastifyPluginAsync = async (app) => {
  app.get<{ Params: { configId: string } }>(
    '/configurations/:configId/identity',
    async (request, reply) => {
      ...
      return {
        ...
        featureFlags: view.config.featureFlags,
        correlationId: request.correlationId,
      };
    },
  );
  ...
  app.put<{
    ...
    Body: { identity: unknown; note?: string };
  }>('/configurations/:configId/identity', async (request, reply) => {
    const access = await requireEdit(app, request, request.params.configId);
    if (!access.ok) return reply.status(access.status).send(access.body);

    let parsed: IdentityPreferences;
    try {
      parsed = IdentityPreferencesSchema.parse(request.body?.identity);
    } catch {
      return reply.status(400).send(
        createApiError({
          code: 'VALIDATION_FAILED',
          message: 'Invalid identity preferences',
          correlationId: request.correlationId,
    ...
    const updated = await app.configStore.update(request.params.configId, {
      ...
      note: request.body?.note ?? 'identity:update',
    });

    return {
      identity: updated!.config.identity,
      correlationId: request.correlationId,
    };
  });
};
