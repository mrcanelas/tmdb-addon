...
async function getSearch(type, language, query, rpdbkey) {
  if (type === "movie") {
    const searchMovie = [] 
    await moviedb
      ...
      .then((res) => {
        res.results.map((el) => {
          searchMovie.push({
            id: `tmdb:${el.id}`,
            name: `${el.title}`,
            poster: rpdbkey ? `https://api.ratingposterdb.com/${rpdbkey}/tmdb/poster-default/${type}-${el.id}.jpg` : `https://image.tmdb.org/t/p/w500${el.poster_path}`,
            type: `${type}`,
