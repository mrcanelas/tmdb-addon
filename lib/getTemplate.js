...
async function landingTemplate(manifest) {
  ...
  const languagesHTML = languages
    .map((language) =>
      language.iso_639_1 === "en-US"
        ? `<option value="${language.iso_639_1}" selected>${language.name} (${language.iso_639_1})</option>`
        : `<option value="${language.iso_639_1}">${language.name} (${language.iso_639_1})</option>`
    )
    .join("\n");
