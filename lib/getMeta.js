...
async function getMeta(type, language, tmdbId, host) {
  if (type === "movie") {
    const meta = await moviedb
      ...
      .then(async (res) => {
        ...
        return {
          ...
          imdbRating: res.vote_average.toFixed(1),
          name: res.title,
          ...
          links: new Array(
            ...
            ...Utils.parseGenreLink(res.genres, host, type, language),
    ...
    const meta = await moviedb
      ...
      .then(async (res) => {
        ...
        const videos = [];
        mapEpisodes.map((x) => x.map((y) => y.map((z) => videos.push(z))));
        return {
          cast: Utils.parseCast(res.credits),
          country: Utils.parseCoutry(res.production_countries),
          description: res.overview,
