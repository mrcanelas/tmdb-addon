...
async function getMeta(type, language, tmdbId) {
  if (type === "movie") {
    ...
    let meta = {
      ...
      type: type,
      name: `${title}`,
      slug: `movie/${title.toLowerCase().split(' ').join('-')}-${(imdb_id == null) ? null : imdb_id.replace('tt', '')}`,
      ...
      runtime: (runtime !== null) ? `${runtime} min` : '',
      background: `https://image.tmdb.org/t/p/original${backdrop_path}`,
      logo: logo,
      releaseInfo: (release_date !== null) ? `${release_date.substr(0, 4)}` : '',
      trailers: videos,
      behaviorHints: {
        defaultVideoId: imdb_id
      },
      videos: [
        {
          id: imdb_id,
    ...
    function year() {
      if (status === "Ended") {
        return (first_air_date !== null) ? first_air_date.substr(0, 5) + last_air_date.substr(0, 4) : '';
      } else {
        return (first_air_date !== null) ? first_air_date.substr(0, 5) : '';
      }
    }
    ...
    let meta = {
      ...
      type: type,
      name: `${name}`,
      slug: `series/${name.toLowerCase().split(' ').join('-')}-${(imdb_id == null) ? null : imdb_id.replace('tt', '')}`,
      imdbRating: `${vote_average}`,
      genres: genero,
      cast: cast,
      writer: writer,
      ...
      posterShape: "regular",
      description: `${overview}`,
      runtime: (episode_run_time !== null) ? `${episode_run_time.slice(0, 1)} min` : '',
      background: `https://image.tmdb.org/t/p/original${backdrop_path}`,
      logo: logo,
      releaseInfo: year(),
      trailers: videos,
