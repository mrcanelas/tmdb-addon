  ...
  if (type === "movie") {
    const meta = await moviedb
      ...
      .then(async (res) => {
        ...
        const metas = {
          ...
          type: type,
          name: res.title ? res.title : "",
          slug: `movie/${
            res.title ? res.title.toLowerCase().replace(/ /g, "-") : ""
          }-${res.imdb_id ? res.imdb_id.replace("tt", "") : ""}`,
          imdbRating: res.vote_average ? res.vote_average : "",
          released: res.release_date ? new Date(res.release_date).toISOString : "",
          genres: res.genre
            ? res.genres.map((el) => {
                return el.name;
              })
          ...
          cast: res.credits.cast
            ? res.credits.cast.slice(0, 4).map((el) => {
                return el.name;
              })
            : [],
          ...
          poster: `https://image.tmdb.org/t/p/w500${
            res.poster_path ? res.poster_path : ""
          }`,
          posterShape: "regular",
          ...
          ? `${res.release_date.substr(0, 4)}`
          : "",
          releaseInfo: res.release_date
            ? `${res.release_date.substr(0, 4)}`
            : "",
          trailers: res.videos.results
            ...
            : [],
          behaviorHints: {
            defaultVideoId: res.imdb_id ? res.imdb_id : "",
          },
          videos: [
            {
              id: res.imdb_id ? res.imdb_id : "",
              title: "",
    ...
    const meta = await moviedb
      ...
      .then(async (res) => {
        ...
        const metas = {
          id: `tmdb:${tmdbId}`,
          imdb_id: res.external_ids.imdb_id ? res.external_ids.imdb_id : "",
          type: type,
          name: res.name ? res.name : "",
          released: res.first_air_date ? new Date(res.first_air_date).toISOString : "",
          slug: `series/${
            res.name ? res.name.toLowerCase().replace(/ /g, "-") : ""
          }-${
            res.external_ids.imdb_id
              ? res.external_ids.imdb_id.replace("tt", "")
              : ""
          }`,
          status: res.in_production == true ? "Continuing" : "Ended",
          imdbRating: res.vote_average ? res.vote_average : "",
          genres: res.genres
            ? res.genres.map((el) => {
                return el.name;
          ...
          cast: res.credits.cast
            ...
            : [],
          writer: res.created_by
            ? res.created_by.map((el) => {
                return el.name;
              })
            : [],
          poster: `https://image.tmdb.org/t/p/w500${
